...
export default function webpackConfigFactory(buildOptions) {
  ...
  const webpackConfig = {
    ...
    resolve: {
      ...
      modules: [ path.resolve(appRootDir.get(), 'src/shared'), 'node_modules' ]
    },
    ...
    plugins: removeEmpty([
      ...
      new webpack.DefinePlugin({
        ...
        'process.env.MIXPANEL_TOKEN': isServer ? 'process.env.MIXPANEL_TOKEN' : 'window.__CLIENT_CONFIG__.env.MIXPANEL_TOKEN'
      }),
          ...
          merge(
            ...
            ifProdClient(() => ({
              loader: ExtractTextPlugin.extract({
                ...
                loader: ['css-loader', `sass-loader?{"includePaths":["${path.resolve(appRootDir.get(), 'src/shared')}","${path.resolve(appRootDir.get(), 'node_modules')}"]}`],
              }),
            })),
            // When targetting the server we use the "/locals" version of the
            // css loader, as we don't need any css files for the server.
            ifNode({
              loader: ['css-loader/locals', `sass-loader?{"includePaths":["${path.resolve(appRootDir.get(), 'src/shared')}","${path.resolve(appRootDir.get(), 'node_modules')}"]}`],
            }),
          ),
        ),
